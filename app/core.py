...
def fetch_artifact(artifact_type: str, artifact_id: str) -> Optional[Artifact]:
...
def list_artifacts(query: ArtifactQuery) -> Dict[str, Any]:
    ...
    filtered: List[Artifact] = [
        item
        for item in sorted(
            _STORE.values(), key=lambda art: (art.metadata.type, art.metadata.name)
        )
    ...
    if query.name:
        ...
        filtered = [
            ...
            if name_lower in item.metadata.name.lower()
        ]
...
def _validate_artifact_data(artifact_type: str, data: Any) -> Dict[str, Any]:
    ...
    if artifact_type == "model":
        model_link_raw = (
            normalized.get("model_link")
            or normalized.get("model_url")
...
def enumerate_artifacts_route() -> Response:
    """Enumerate artifacts matching ArtifactQuery; returns list with offset header for pagination."""
    _require_auth()
    payload = _json_body()
...
def ingest_hf_route() -> Response:
    ...
    non_latency = ["bus_factor","code_quality","dataset_quality",
                   "dataset_and_code_score","license","performance_claims","ramp_up_time"]
    failures = {k: rating.scores.get(k, 0.0) for k in non_latency if (rating.scores.get(k,0.0) or 0.0) < 0.5}
    if failures:
    ...
    art = Artifact(
        ...
        data={"model_link": f"https://huggingface.co/{hf_id}", "path": f"uploads/{hf_id.replace('/','_')}.zip"}
    )
    save_artifact(art)
    return jsonify({"ingestible": True, "artifact": artifact_to_dict(art), "scores": rating.scores}), 201
