...
from app.s3_adapter import (
    S3Storage, USE_S3, S3_BUCKET, S3_REGION, S3_PREFIX
)
...
class TestS3StorageInitialization:
    ...
    def test_s3storage_init_enabled_no_bucket(self):
        ...
        with patch('app.s3_adapter.USE_S3', True), \
             patch('app.s3_adapter.S3_BUCKET', None), \
...
class TestS3StorageFileOperations:
    ...
    @patch('app.s3_adapter.S3_KMS_KEY_ID', 'arn:aws:kms:us-east-1:123456789012:key/12345678-1234-1234-1234-123456789012')
    ...
    def test_put_file_with_encryption_and_acl(self, mock_client):
        ...
        mock_client.put_object.return_value = {'VersionId': 'v123'}
        mock_client.head_object.return_value = {'ContentLength': 1024, 'ContentType': 'application/zip'}
        
        file_obj = io.BytesIO(b'encrypted content')
        result = self.storage.put_file(file_obj, 'encrypted.zip', 'application/zip')
    ...
    def test_generate_presigned_url_with_version(self, mock_client):
        ...
        url = self.storage.generate_presigned_url('test.txt', version_id='v123')
