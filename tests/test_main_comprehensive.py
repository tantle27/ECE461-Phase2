...
class TestCalculateNetScore:
    ...
    def test_calculate_score_bounds(self):
        ...
        metrics = {m: 2.0 for m in ["license", "ramp_up_time", "dataset_and_code_score", 
                                    "performance_claims", "bus_factor", "code_quality", "dataset_quality"]}
        score = calculate_net_score(metrics)
        assert score == 1.0
        
        # Test under 0.0 (should floor at 0.0)
        metrics = {m: -1.0 for m in ["license", "ramp_up_time", "dataset_and_code_score",
                                     "performance_claims", "bus_factor", "code_quality", "dataset_quality"]}
        score = calculate_net_score(metrics)
        assert score == 0.0
